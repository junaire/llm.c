...
int main() {
  ...
  // trained on) int T = 64; // sequence length 64 (i.e. each sequence is 64
  // tokens long). must be <= maxT, which is 1024 for GPT-2 const char*
  // tiny_shakespeare_train =
  // "dev/data/tinyshakespeare/tiny_shakespeare_train.bin"; const char*
  // tiny_shakespeare_val = "dev/data/tinyshakespeare/tiny_shakespeare_val.bin";

  // GPT2* model = gpt2_create("gpt2_124M.bin");
  // DataLoader* train_loader = dataloader_create(tiny_shakespeare_train, B, T,
  // 0, 1, 1); DataLoader* val_loader = dataloader_create(tiny_shakespeare_val,
  // B, T, 0, 1, 0); printf("train dataset num_batches: %zu\n",
