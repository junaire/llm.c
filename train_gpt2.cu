...
// dataloader_free defines: evalloader_init, evalloader_reset,
// evalloader_next_batch, evalloader_free
...
// defines: CUBLAS_LOWP, cublasCheck, cublaslt_workspace_size,
// cublaslt_workspace defines: cublas_compute, cublaslt_handle, cublas_handle
...
// defines: layernorm_forward, residual_forward, fused_residual_forward5,
// layernorm_backward
#include "llmc/layernorm.cuh"
// defines: matmul_cublaslt, matmul_forward, matmul_backward, gelu_forward,
// gelu_backward_inplace
#include "llmc/matmul.cuh"
#ifdef ENABLE_CUDNN
// defines: create_cudnn, destroy_cudnn, attention_forward_cudnn,
...
void set_zero_configs(MultiGpuConfig *multi_gpu_config, int zero_stage,
  ...
  if (zero_stage == 0) {
    ...
    if (total_parameters % multi_gpu_config->num_processes != 0) {
      printf0("| Zero Optimization is disabled, Can't equally partition "
              "parameters          |\n");
      multi_gpu_config->zero_stage = 0;
    } else {
      multi_gpu_config->zero_stage = 1;
      multi_gpu_config->shard_num_parameters =
          total_parameters / multi_gpu_config->num_processes;
    }
  } else {
    printf0("| Disabling Zero Optimization, Zero Stage2 and Stage3 are not yet "
            "supported  |\n");
    multi_gpu_config->zero_stage = 0;
  }
}
...
static_assert(sizeof(ParameterTensors) ==
                  NUM_PARAMETER_TENSORS * sizeof(void *),
              "Inconsistent sizes!");
...
void fill_in_parameter_sizes(size_t *param_sizes, size_t *param_sizeof,
  ...
  // populate the parameter sizes in bytes (all the same for now, keeping for
  // future use)
...
typedef struct {
  ...
  floatX *lnf; // (B, T, C);   if LN recomputation is enabled (-r 2 and above),
               // will be used for _all_ layernorms
  float *lnf_mean; // (B, T)
  float *lnf_rstd; // (B, T)
  ...
  // general scratchpad buffer. Allocation is made large enough to hold (B, T,
  // 3C), (B, NH, T, T), and (B, T, V) shaped tensors.
  floatX *output;
...
void fill_in_activation_sizes(const ActivationTensors *data,
                              ...
                              size_t B, size_t T, GPT2Config config,
                              int recompute) {
  size_t Vp = config.padded_vocab_size;
  size_t L = config.num_layers;
  ...
  // if recompute >= 1 then we will recompute the layernorm forward activation
  ...
  // if recompute >= 1 then we will recompute gelu_forward during backward and
  // use this as scratch buffer
  tensors[11] = TENSOR_SPEC(data->fch_gelu, (recompute < 1) ? L * B * T * 4 * C
                                                            : B * T * 4 * C);
...
void *
malloc_and_point_activations(TensorSpec (&tensors)[NUM_ACTIVATION_TENSORS]) {
  size_t bytes = 0;
  ...
  // cudaMalloc does not guarantee initial memory values so we memset the
...
typedef struct {
  ...
  float mean_loss; // after the last backward micro-batch, will be populated
                   // with mean loss across all GPUs and micro-steps
  float *accumulated_mean_loss; // GPU buffer used to accumulate loss across
                                // micro-steps
  float *cpu_losses; // CPU buffer to copy the losses to, allocated with
                     // cudaMallocHost
  unsigned long long
      rng_state;          // the RNG state for seeding stochastic rounding etc.
  int use_master_weights; // keep master weights copy in float for optim update?
                          // 0|1
  int gelu_fusion;        // fuse gelu via cuBLASLt (0=none, 1=forward,
                          // 2=forward+backward)
  int recompute; // recompute gelu | layernorm forward during model backward?
                 // 0|1|2
  // todo - if other functions need cpu scratch buffers in the future, reuse as
  // generic scratch?
  int *workload_indices; // encoder_backward, B*T*num_c_groups (int)
  int4 *bucket_info;     // encoder_backward, B*T*num_c_groups (int4) - size for
                         // worst case
} GPT2;
...
void gpt2_init_common(GPT2 *model) {
  ...
  model->rng_state =
      13371337 + multi_gpu_config.process_rank; // used in stochastic rounding
  model->use_master_weights = 1; // safe default: do keep master weights in fp32
  model->recompute = 1;   // good default: recompute gelu but not layernorm
  model->gelu_fusion = 0; // deviceProp.major >= 9 ? 2 : 0; // default: off for
                          // now (default must match main())
}
...
void gpt2_build_from_checkpoint(GPT2 *model, const char *checkpoint_path) {
  ...
  if (PRECISION_MODE == PRECISION_FP16) {
    ...
    // the fp16 weights directly from Python, which we only do for fp32/bf16
    // atm.
    fprintf(stderr,
  ...
  fill_in_parameter_sizes(model->param_elements, model->param_sizeof,
                          model->config);
  ...
  for (int i = 0; i < NUM_PARAMETER_TENSORS; i++) {
    ...
    model->num_parameters_bytes +=
        model->param_elements[i] * model->param_sizeof[i];
  }

  // create memory for model parameters on the device
  assert(model->params_memory == nullptr &&
         "Old model needs to be freed before loading from checkpoint again");
  ...
  // only return from this function once we are certain the params are ready on
  // the GPU
...
void gpt2_build_from_random(GPT2 *model, int depth) {
  ...
  // parameterize the size of gpt2 based only on the depth of the model
  // (num_layers)
  ...
  // allocate and random init the memory for all the parameters with GPT-2
  // schema weights ~N(0, 0.02), biases 0, c_proj weights ~N(0, 0.02/(2*L)**0.5)
  // NOTE: assuming all parameters are of the type floatX, could be relaxed
  // later
  ...
  // we have to init all these tensors exactly in the order that PyTorch
  // initializes them so that we can match them up and get correctness and
  ...
  for (int l = 0; l < L; l++) {
    ...
    for (int i = 0; i < NUM_PARAMETER_TENSORS; i++) {
      ...
      if (l == 0 && (i == 2 || i == 8 ||
                     i == 14)) { // only at l = 0 to init these just once
      ...
      if ((l == 0 &&
           (i == 0 || i == 1)) // only at l = 0, init the wte and wpe tensors
          || i == 4 || i == 6 || i == 10 || i == 12) {
        ...
        // in GPT-2, the projections back into the residual stream are
        // additionally scaled by 1/sqrt(2*L) for training stability
  ...
  // copy them to GPU
  cudaCheck(cudaMemcpy(model->params_memory, params_memory_cpu,
                       model->num_parameters_bytes, cudaMemcpyHostToDevice));
...
void gpt2_forward(GPT2 *model, const int *inputs, size_t B, size_t T) {
  ...
  if (model->acts_memory == NULL) {
    ...
    cudaCheck(
        cudaMalloc(((void **)&model->accumulated_mean_loss), sizeof(float)));
    cudaCheck(
  ...
  layernorm_forward((model->recompute < 2) ? acts.ln1 : acts.lnf, acts.ln1_mean,
                    acts.ln1_rstd, acts.encoded, params.ln1w, params.ln1b, B, T,
                    C, main_stream);
  ...
  for (int l = 0; l < L; l++) {
    ...
    floatX *l_ln1 =
        (model->recompute < 2) ? acts.ln1 + l * B * T * C : acts.lnf;
    floatX *l_qkvr = acts.qkvr + l * B * T * 3 * C;
    floatX *l_atty = acts.atty + l * B * T * C;
    floatX *l_residual2 = acts.residual2 + l * B * T * C;
    floatX *l_ln2 =
        (model->recompute < 2) ? acts.ln2 + l * B * T * C : acts.lnf;
    float *l_ln2_mean = acts.ln2_mean + l * B * T;
    float *l_ln2_rstd = acts.ln2_rstd + l * B * T;
    floatX *l_fch = acts.fch + l * B * T * 4 * C;
    // reuse the same activation buffer at each layer, as we'll re-compute the
    // gelu during backward very useful because we dramatically reduce VRAM
    // usage, and may be able to fit larger batch size
    floatX *l_fch_gelu = (model->recompute < 1)
                             ? acts.fch_gelu + l * B * T * 4 * C
                             : acts.fch_gelu;
    floatX *l_residual3 = acts.residual3 + l * B * T * C;
    floatX *scratch =
        (floatX *)
            acts.output; // used for non-cudnn attention, fcproj, attproj, etc.

// now do the forward pass
#ifdef ENABLE_CUDNN
    float *l_att =
        (float *)acts.att + l * B * NH * T; // cuDNN needs a smaller FP32 tensor
    ...
    matmul_forward_cublaslt(scratch, l_atty, l_attprojw, l_attprojb, B, T, C, C,
                            main_stream);
    fused_residual_forward5(l_residual2, l_ln2, l_ln2_mean, l_ln2_rstd,
                            residual, scratch, l_ln2w, l_ln2b, B * T, C,
                            main_stream);
    matmul_forward_cublaslt(l_fch_gelu, l_ln2, l_fcw, l_fcb, B, T, C, 4 * C,
                            main_stream, l_fch, model->gelu_fusion);
    matmul_forward_cublaslt(scratch, l_fch_gelu, l_fcprojw, l_fcprojb, B, T,
                            4 * C, C, main_stream);
    // OK, fusion across blocks.
    if (l + 1 != L) {
      floatX *l_ln1 =
          (model->recompute < 2) ? acts.ln1 + (l + 1) * B * T * C : acts.lnf;
      ...
      fused_residual_forward5(l_residual3, l_ln1, l_ln1_mean, l_ln1_rstd,
                              l_residual2, scratch, l_ln1w, l_ln1b, B * T, C,
                              main_stream);
    } else {
      fused_residual_forward5(l_residual3, acts.lnf, acts.lnf_mean,
                              acts.lnf_rstd, l_residual2, scratch, params.lnfw,
                              params.lnfb, B * T, C, main_stream);
    }
  }
...
// evals. In particular it populates cpu_losses with loss at each token. Some of
// the evals (e.g. HellaSwag) require the per-token losses, which are produced
// here.
float gpt2_validate(GPT2 *model, const int *inputs, const int *targets,
                    size_t B, size_t T) {
  assert(targets != NULL);
  // forward the model itself
  gpt2_forward(model, inputs, B, T);
  // convenience shortcuts, size_t instead of int so that pointer arithmetics
  // don't overflow
  const size_t V = model->config.vocab_size;
  const size_t Vp = model->config.padded_vocab_size;
...
void gpt2_backward_and_reduce(GPT2 *model, int *inputs, const int *targets,
  ...
  if (model->grads_memory == NULL) {
    ...
    size_t num_c_groups =
        CEIL_DIV(model->config.channels, (WARP_SIZE * x128::size));
    assert((size_t)(model->batch_size * model->seq_len) * num_c_groups <
           (1ULL << 31ULL)); // todo - maybe an issue for llama3-400B(?)
    model->workload_indices = (int *)mallocCheck(
        sizeof(int) * model->batch_size * model->seq_len * num_c_groups);
    model->bucket_info = (int4 *)mallocCheck(sizeof(int4) * model->batch_size *
                                             model->seq_len * num_c_groups);
  }

  // on the first micro-step zero the gradients, as we're about to += accumulate
  // into them
  if (micro_step == 0) {
    // there are currently two state vars during the gradient accumulation inner
    // loop: 1) the losses accumulate += into acts.losses, reset here 2) the
    // gradients accumulate += into grads_memory, reset here
  ...
  // accumulate the losses inside acts.losses, and kick off the backward pass
  // inside the fused classifier
  NvtxRange classifier_and_loss_range("classifier_and_loss");
  const float dloss =
      1.0f /
  ...
  // reset residual stream gradients (put here to work with gradient
  // accumulation)
  floatX *dresidual =
  ...
  // the batch next: backward the classifier matmul
  matmul_backward(model->acts.scratch_bt4c, grads.wte, NULL, acts.output,
                  acts.lnf, params.wte, NULL, B, T, C, Vp, main_stream);
  // backward the final layernorm
  floatX *residual =
      acts.residual3 + (L - 1) * B * T * C; // last residual is in residual3
  layernorm_backward(dresidual, grads.lnfw, grads.lnfb, scratchF,
                     model->acts.scratch_bt4c, residual, params.lnfw,
                     acts.lnf_mean, acts.lnf_rstd, B, T, C, main_stream);

  // from this point on, we no longer need the values stored in the last
  // residual, so we can reuse that memory as generic scratch for backward
  // computations
  ...
  for (int l = L - 1; l >= 0; l--) {
    ...
    // re-using this memory in every Transformer block as we calculate backward
    // pass
    ...
    matmul_backward(dl_bt4c, dl_fcprojw, dl_fcprojb, dresidual, l_fch_gelu,
                    l_fcprojw, scratchF, B, T, 4 * C, C, main_stream,
                    l_fch_pre_gelu, model->gelu_fusion);
    if (model->recompute >= 2) {
      // same as gelu above, l_ln1 and l_ln2 are just buffers if recompute >= 2,
      // recompute them here on demand
      layernorm_forward(l_ln2, l_ln2_mean, l_ln2_rstd, l_residual2, l_ln2w,
                        l_ln2b, B, T, C, main_stream);
    }
    matmul_backward(dl_btc, dl_fcw, dl_fcb, dl_bt4c, l_ln2, l_fcw, scratchF, B,
                    T, C, 4 * C, main_stream);
    // layernorm backward does += to the dresidual, so it correctly accumulates
    // grad from the MLP block above
    layernorm_backward(dresidual, dl_ln2w, dl_ln2b, scratchF, dl_btc,
                       l_residual2, l_ln2w, l_ln2_mean, l_ln2_rstd, B, T, C,
                       main_stream);
    ...
    float *l_att =
        (float *)acts.att + l * B * NH * T; // cuDNN needs a smaller FP32 tensor
    attention_backward_cudnn(dl_bt4c, dl_btc, l_qkvr, l_atty, (float *)l_att, B,
                             T, NH, C, main_stream);
#else
    floatX *l_att = acts.att + l * B * NH * T * T;
    // we need B x T x (4)C buffers. l_atty and l_fch aren't needed anymore at
    // this point, so reuse their memory
    floatX *buffer_a = l_atty;
    floatX *buffer_b =
        l_fch_pre_gelu; // this is B x T x 4C, so even larger than what we need
    attention_backward(dl_bt4c, buffer_b, scratchX, buffer_a, dl_btc, l_qkvr,
                       l_att, B, T, C, NH, main_stream);
#endif
    if (model->recompute >= 2) {
    ...
    matmul_backward(dl_btc, dl_qkvw, dl_qkvb, dl_bt4c, l_ln1, l_qkvw, scratchF,
                    B, T, C, 3 * C, main_stream);
    // layernorm backward does += to dresidual, so it correctly accumulates
    // gradient for the Attention block above
    ...
    if (last_step) {
      ...
      const size_t nelem[] = {C, C, 3 * C * C, 3 * C, C * C,     C,
                              C, C, 4 * C * C, 4 * C, C * 4 * C, C};
      multi_gpu_async_reduce_gradient(pointers, nelem, &multi_gpu_config,
                                      main_stream);
    }
  }
  encoder_backward(grads.wte, grads.wpe, scratchX, model->workload_indices,
                   model->bucket_info, dresidual, model->inputs, inputs, B, T,
                   C, random_u32(&model->rng_state), main_stream);
  ...
  if (last_step) {
    ...
    global_sum_deterministic(model->accumulated_mean_loss, acts.losses, B * T,
                             main_stream);
// reduce loss across GPUs to a single, final float across all microsteps and
// GPUs
#if MULTI_GPU
...
// Compute sum of a single CPU value across all GPU processes. No-op when
...
float multi_gpu_cpu_float_sum(float value, MultiGpuConfig *multi_gpu_config) {
  ...
  *unified_buffer = value;
  ncclCheck(ncclAllReduce(unified_buffer, unified_buffer, sizeof(float),
                          ncclFloat, ncclSum, multi_gpu_config->nccl_comm,
...
ShardInfo gpt2_get_tensor_at_layer(const GPT2 *model, int layer_id,
  ...
  // if we are in the transformer block, we need to additionally offset by the
  // layer id
...
float gpt2_calculate_grad_norm(GPT2 *model, MultiGpuConfig *multi_gpu_config) {
  ...
  if (multi_gpu_config->zero_stage == 1) {
    ...
    // so we only calculate the grad norm at the grads_memory belonging to the
    // local shards
    for (int i = 0; i < NUM_PARAMETER_TENSORS; i++) {
      ...
      if ((i < 2 || i > 13)) {
        global_norm_squared(grad_norm_squared, grads_memory + offset,
                            shard.size, 0, 1, max_num_block_sums, is_first_pass,
                            main_stream);
      } else {
        global_norm_squared(grad_norm_squared, grads_memory + offset,
                            shard.size, tensor.size, model->config.num_layers,
                            max_num_block_sums, is_first_pass, main_stream);
      }
    }
    global_sum_deterministic(grad_norm_squared, grad_norm_squared,
                             max_num_block_sums, main_stream);
#if MULTI_GPU
    // further sum the (partial) squared norm across all GPUs
    ncclCheck(ncclAllReduce(grad_norm_squared, grad_norm_squared, sizeof(float),
                            ncclFloat, ncclSum, multi_gpu_config->nccl_comm,
                            main_stream));
#endif
  } else {
    // in regular DDP, backward has averaged the gradients across all GPUs
    // so each GPU can compute the squared norm over the whole grad vector, with
    // no added comms needed
    global_norm_squared(grad_norm_squared, grads_memory, model->num_parameters,
                        0, 1, max_num_block_sums, true, main_stream);
    global_sum_deterministic(grad_norm_squared, grad_norm_squared,
                             max_num_block_sums, main_stream);
  }
  cudaCheck(cudaMemcpy(&grad_norm_squared_cpu, grad_norm_squared, sizeof(float),
                       cudaMemcpyDeviceToHost));
...
void gpt2_update(GPT2 *model, float learning_rate, float beta1, float beta2,
  ...
  // keep in mind that optimizer sharding (ZeRO-1) assigns different parameters
  // to different GPUs so we may not be responsible for the entire parameter
  // tensor also, this function was very simple a while back but become very
  // complex, only because we want to selectively weight decay some, but not all
  // tensors :(
  // TODO: revisit and probably refactor this entire function
  NVTX_RANGE_FN();
  size_t shard_num_parameters =
      multi_gpu_config
          ->shard_num_parameters; // num parameters we are responsible for

  // lazily allocate m,v memory and master weights (usually on the first
  // iteration)
  if (model->m_memory == NULL) {
    ...
    cudaCheck(cudaMalloc((void **)&model->v_memory,
                         shard_num_parameters * sizeof(float)));
    cudaCheck(
        cudaMemset(model->m_memory, 0, shard_num_parameters * sizeof(float)));
    cudaCheck(
        cudaMemset(model->v_memory, 0, shard_num_parameters * sizeof(float)));
  }
  ...
  if (model->use_master_weights == 1 && model->master_weights == NULL) {
    printf0("allocating %zu MiB for master copy of params\n",
            (shard_num_parameters * sizeof(float)) >> 20);
    cudaCheck(cudaMalloc((void **)&model->master_weights,
                         shard_num_parameters * sizeof(float)));
  ...
  for (int i = 0; i < NUM_PARAMETER_TENSORS; i++) {
    ...
    // we only want to weight decay the 2D tensors and leave all 1D tensors
    // alone in particular this also decays the embedding weights, but this is
    // ok:
    // - the token embeddings are weight shared and participate in the final
    ...
    ptrdiff_t opt_state_offset = multi_gpu_config->zero_stage < 1
                                     ? local_offset_full
    ...
    if (init_master_weights) {
      ...
      copy_and_cast_kernel<<<dim3(grid_size, num_layers), 512, 0,
                             main_stream>>>(master_ptr, param_ptr, shard.size,
    ...
    if (multi_gpu_config->zero_stage == 1) {
      ...
      for (int l = 0; l < num_layers; ++l) {
        ...
        ncclCheck(ncclAllGather(
            param_ptr + l * tensor.size,
            (floatX *)model->params_memory + tensor.offset + l * tensor.size,
            shard.size, ncclFloatX, multi_gpu_config->nccl_comm,
            multi_gpu_config->nccl_stream));
      }
...
float gpt2_estimate_mfu(GPT2 *model, int num_tokens, float dt) {
  ...
  but these are very small terms. Also keep in mind that we would want to
  exclude the token embedding weights, but in GPT-2 these are weight shared, so
  they participate in the classifier matmul, so they are correct to be included
  in N. Note 2: The first term (6 * N) in flops_per_token is all weight matmuls,
  the second is the attention matmul, which is also usually a small
  contribution.
  */
  ...
  float flops_promised =
      get_flops_promised(deviceProp.name, PRECISION_MODE) * 1e12f;
  if (flops_promised < 0) {
...
void common_start(bool override_enable_tf32 = true,
  ...
  bool enable_tf32 = PRECISION_MODE == PRECISION_FP32 &&
                     deviceProp.major >= 8 && override_enable_tf32;
  cublas_compute =
...
void save_state(const char *filename, int step, GPT2 *model,
                DataLoader *loader) {
  ...
  state_header[4] =
      model->use_master_weights; // whether we're using fp32 master weights
  ...
  *((unsigned long long *)&state_header[20]) =
      model->rng_state; // random number generator state
  // dataloader state, start at 30 to leave some padding
  *((size_t *)&state_header[30]) =
      loader->current_shard_idx; // shard of the dataset
  *((size_t *)&state_header[32]) =
      loader->current_sample_idx; // position in shard
  ...
  device_to_file(state_file, model->m_memory,
                 shard_num_parameters * sizeof(float), IO_BUF_SIZE,
                 main_stream);
  device_to_file(state_file, model->v_memory,
                 shard_num_parameters * sizeof(float), IO_BUF_SIZE,
                 main_stream);
  ...
  if (loader->should_shuffle) {
    fwriteCheck(&loader->glob_result.gl_pathc, sizeof(size_t), 1,
                state_file); // number of shards
...
void load_state(int *step, GPT2 *model, DataLoader *loader,
                const char *filename) {
  ...
  assert(state_header[2] ==
         multi_gpu_config.num_processes); // number of processes
  assert(state_header[3] ==
         multi_gpu_config.process_rank); // rank of this process
  int use_master_weights =
      state_header[4]; // whether we're using fp32 master weights
  int should_shuffle = state_header[5]; // shuffle state of the dataloader
  *step = state_header[10];             // step of the optimization
  model->rng_state = *(
      (unsigned long long *)&state_header[20]); // random number generator state
  size_t current_shard_idx = *((size_t *)&state_header[30]); // shard index
  size_t current_sample_idx =
      *((size_t *)&state_header[32]); // position in shard
  ...
  if (use_master_weights == 1 && !model->use_master_weights) {
    printf0("Warning: Master weights are present in state, but not enabled for "
            "current run.");
  ...
  file_to_device(model->m_memory, state_file,
                 shard_num_parameters * sizeof(float), IO_BUF_SIZE,
                 main_stream);
  file_to_device(model->v_memory, state_file,
                 shard_num_parameters * sizeof(float), IO_BUF_SIZE,
                 main_stream);
  if (model->use_master_weights) {
  ...
  if (should_shuffle == 1) {
    ...
    loader->shard_indices =
        (int *)mallocCheck(loader->glob_result.gl_pathc * sizeof(int));
    freadCheck(loader->shard_indices, sizeof(int), loader->glob_result.gl_pathc,
               state_file);
    ...
    loader->intra_shard_indices =
        (int *)mallocCheck(loader->shard_num_samples * sizeof(int));
    freadCheck(loader->intra_shard_indices, sizeof(int),
               loader->shard_num_samples, state_file);
...
void write_checkpoint(const char *output_log_dir, int step, GPT2 *model,
                      DataLoader *train_loader,
                      MultiGpuConfig *multi_gpu_config) {
  // a checkpoint contains: model weights, optimizer/dataloader state, and a
  // DONE file
  ...
  if (rank == 0) {
    snprintf(filename_buffer, sizeof(filename_buffer), "%s/model_%08d.bin",
             output_log_dir, step);
  ...
  if (rank == 0) {
    snprintf(filename_buffer, sizeof(filename_buffer), "%s/DONE_%08d",
             output_log_dir, step);
...
void delete_checkpoint(const char *output_log_dir, int step,
                       MultiGpuConfig *multi_gpu_config) {
...
// note that "state" refers to things not already saved in the model checkpoint
// file
...
void error_usage() {
  ...
  fprintf(stderr, "  -e <string> input from model at this filename (default = "
                  "gpt2_124M_bf16.bin)\n");
  fprintf(stderr,
          "  -o <string> output log dir (default = NULL, no logging)\n");
  fprintf(stderr, "  -n <int>    write optimization checkpoints every how many "
                  "steps? (default 0, don't)\n");
  fprintf(stderr, "  -nk <int>   max number of checkpoints to keep in the "
                  "directory, removing old ones (0 = disable, default)\n");
  fprintf(stderr, "  -nm <int>   every how many step checkpoints are "
                  "considered major? major checkpoints never get deleted.\n");
  fprintf(stderr, "  -y <int>    resume optimization found inside output log "
                  "dir? (0=restart/overwrite, 1=resume/append)\n");
  // token layout for each step of the optimization
  fprintf(stderr,
          "  -b <int>    (per-GPU, micro) batch size B (default = 4)\n");
  fprintf(stderr, "  -t <int>    sequence length T (default = 1024)\n");
  fprintf(stderr, "  -d <int>    total desired batch size (default = B * T * "
                  "num_processes, i.e. no grad accumulation\n");
  // workload (number of steps)
  fprintf(stderr, "  -x <int>    max_steps of optimization to run (-1 "
                  "(default) = disable, run 1 epoch)\n");
  // optimization
  ...
  fprintf(stderr, "  -u <int>    learning rate warmup iterations (default = 0, "
                  "no warmup)\n");
  fprintf(stderr, "  -q <float>  learning rate decay: final fraction, at end "
                  "of training (default = 1.0 (no decay))\n");
  ...
  fprintf(stderr, "  -sg <float> outlier stability: skip update if grad_norm "
  ...
  // numerics
  fprintf(stderr, "  -f <int>    enable_tf32 override (default: 1, set to 0 to "
                  "disable tf32)\n");
  fprintf(stderr, "  -w <int>    keep f32 copy of weights for the optimizer? "
                  "(default: 1)\n");
  fprintf(stderr,
          "  -ge <int>   gelu fusion: 0=none, 1=forward, 2=forward+backward "
          "(default: 2 for >=SM90, 0 for older GPUs)\n");
  // memory management
  fprintf(stderr, "  -z <int>    zero_stage, Zero Optimization Stage, 0,1,2,3 "
                  "(default = 0)\n");
  fprintf(stderr, "  -r <int>    recompute: less memory but less speed. "
                  "(default = 1), 0|1|2 = none,gelu,gelu+ln\n");
  // multi-node settings
  ...
  fprintf(stderr, "  -ps <string> server_ip - used only when nccl_init_method "
                  "is tcp (default = -1)\n");
  fprintf(stderr, "  -pp <string> fs_path - used only when nccl_init_method is "
                  "fs (default = /tmp)\n");
  exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[]) {
  ...
  float final_learning_rate_frac =
      1.0f; // final fraction of learning rate, at end of training
  float weight_decay = 0.0f;
  float skip_update_lossz =
  ...
  char server_ip[256] =
      ""; // used if init_method set to "tcp" -> set to your server ip address
  char fs_path[256] =
      ""; // used if init_method set to "fs" -> set to a shared filesystem path
  ...
  multi_gpu_config =
      multi_gpu_config_init(num_processes, process_rank, gpus_per_node,
                            server_ip, fs_path, nccl_init_method);
  common_start(override_enable_tf32,
               false); // common init code for train/test/profile

  // should do a bit more error checking here
  assert(warmup_iterations >= 0);
  if (output_log_dir != NULL) {
    assert(strlen(output_log_dir) <
           400); // careful bunch of hardcoded snprintf around this
  }
  int tokens_per_fwdbwd =
      B * T *
      multi_gpu_config
          .num_processes; // one micro-batch processes this many tokens
  // calculate sensible default for total batch size as assuming no gradient
  // accumulation
  ...
  // if we're only overfitting a single batch for debugging, let's overfit the
  // first batch from val instead of train split, because val is smaller and
  // faster. (train_gpt2.py does the same)
  ...
  if (resume == 1) {
    ...
    if (resume_max_step == -1) {
      ...
      resuming = 1;
      snprintf(filename_buffer, sizeof(filename_buffer), "%s/model_%08d.bin",
               output_log_dir, resume_max_step);
    }
  }
  ...
  // if load_filename is of the form "dX" where X is an integer (e.g. d12), then
  // we build a random model with the depth of the model specified by X (e.g.
  // 12). otherwise interpret this variable as a checkpoint filename, and load
  // that checkpoint
  ...
  if (resuming == 1) {
    ...
    if (depth > 1 &&
        depth <= 1000) { // we're not going to train models this big right? heh
  ...
  printf0(
      "| weight init method    | %-50s |\n",
      resuming == 1
          ? "intermediate checkpoint"
          : (load_filename[0] == 'd' ? "random" : "OpenAI's GPT-2 checkpoint"));
  ...
  if (train_num_batches == -1) {
    ...
    // the number of (outer loop) steps each process should take for us to reach
    // one epoch
  ...
  if (val_num_batches == -1) {
    ...
    // note that unlike the training loop, there is no gradient accumulation
    // inner loop here
  ...
  // more prints related to allocations from gpt2_build_from_checkpoint down
  ...
  printf0("batch_size B=%d * seq_len T=%d * num_processes=%d and "
          "total_batch_size=%d\n",
  ...
  lr_scheduler_init(&lr_scheduler, lr_scheduler_type, learning_rate,
                    warmup_iterations, train_num_batches,
                    final_learning_rate_frac);

  // some memory for generating samples from the model
  int *gen_tokens = (int *)mallocCheck(B * T * sizeof(int));
  floatX *cpu_logits_raw =
      (floatX *)mallocCheck(model.config.vocab_size * sizeof(floatX));
  float *cpu_logits =
  ...
  for (; step <= train_num_batches; step++) {
    ...
    if (step % val_loss_every == 0 || last_step) {
      ...
      for (int i = 0; i < val_num_batches; i++) {
        ...
        val_loss +=
            gpt2_validate(&model, val_loader.inputs, val_loader.targets, B, T);
      }
    ...
    if (run_hellaswag &&
      ...
      // careful because not all ranks may have the exact same allocation of
      // number of examples
      eval_acc_norm = multi_gpu_cpu_float_sum(eval_acc_norm, &multi_gpu_config);
      printf0("HellaSwag: %d/%d = %f\n", (int)eval_acc_norm,
              eval_loader.num_examples,
    ...
    if ((checkpoint_every > 0 && output_log_dir != NULL && resuming == 0) &&
      ...
      write_checkpoint(output_log_dir, step, &model, &train_loader,
                       &multi_gpu_config);
      // we only keep checkpoints_keep checkpoints on disk to save space
      // so now that we wrote a new checkpoint, delete one old one (unless it is
      // a "major" checkpoint) we only do this is checkpoint keeping is turned
      // on (checkpoints_keep > 0)
      int step_delete = step - checkpoints_keep * checkpoint_every;
      if (checkpoints_keep > 0 && step_delete > 0 &&
          (major_checkpoint_every == 0 ||
           step_delete % major_checkpoint_every != 0)) {
    ...
    // do one training step, doing forward/backward/update on total_batch_size
    // tokens
    ...
    for (int micro_step = 0; micro_step < grad_accum_steps; micro_step++) {
      ...
      // forward pass. note that we pass in grad_accum_steps, which scales down
      // the loss
      gpt2_forward(&model, train_loader.inputs, B, T);
      // backward pass. all model params accumulate gradients with += inside
      // this inner loop
      gpt2_backward_and_reduce(&model, train_loader.inputs,
                               train_loader.targets, grad_accum_steps,
                               micro_step);
    }
    float zloss = (float)(update_detector(
        &loss_outlier_detector, (double)model.mean_loss)); // loss z-score
    ...
    float zgrad = (float)(update_detector(&grad_norm_outlier_detector,
                                          (double)grad_norm)); // grad z-score
    // update the model parameters
    if (isfinite(zloss) && skip_update_lossz != 0.0f &&
        zloss > skip_update_lossz) {
      printf0("skipping update due to loss z-score of %f\n", zloss);
    } else if (isfinite(zgrad) && skip_update_gradz != 0.0f &&
               zgrad > skip_update_gradz) {
      ...
      gpt2_update(&model, step_learning_rate, 0.9f, 0.95f, 1e-8f, weight_decay,
                  grad_scale, step + 1, &multi_gpu_config);
    }
    cudaCheck(cudaEventRecord(end));
    cudaCheck(cudaEventSynchronize(
        end)); // wait for the end event to finish to get correct timings
    // --------------- TRAINING SECTION END -------------------
    // everything that follows now is just diagnostics, prints, logging, etc.

    // todo - move or double-buffer all of this timing logic to avoid idling the
    // GPU at this point!
    float time_elapsed_ms;
    cudaCheck(cudaEventElapsedTime(&time_elapsed_ms, start, end));
    size_t tokens_processed =
        (size_t)multi_gpu_config.num_processes * B * T * grad_accum_steps;
    float tokens_per_second = tokens_processed / time_elapsed_ms * 1000.0f;
    float bias_corrected_ema_tokens_per_second =
        tokens_per_second; // by default set to non-ema version
    if (step > 0) {        // consider the first batch to be a warmup (e.g.
                           // cuBLAS/cuDNN initialisation)
      total_sum_iteration_time_s += time_elapsed_ms / 1000.0f;
      // smooth out the tok/s with an exponential moving average, and bias
      // correct just like in AdamW
      ema_tokens_per_second =
          0.95f * ema_tokens_per_second + 0.05f * tokens_per_second;
      bias_corrected_ema_tokens_per_second =
          ema_tokens_per_second / (1.0f - powf(0.95f, step));
    }
    float mfu = gpt2_estimate_mfu(&model, B * T * grad_accum_steps,
    ...
    printf0("step %4d/%d | loss %7.6f (%+.2fz)| norm %6.4f (%+.2fz)| lr %.2e | "
            ...
            bias_corrected_ema_tokens_per_second);
    logger_log_train(&logger, step, model.mean_loss, step_learning_rate,
                     grad_norm);
...
GPT2 *gpt2_create(const char *checkpoint) {
  GPT2 *gpt2 = (GPT2 *)mallocCheck(sizeof(GPT2));
  gpt2_init_common(gpt2);
  gpt2_build_from_checkpoint(gpt2, checkpoint);
  return gpt2;
}
...
void gpt2_train(GPT2 *model, DataLoader *train_loader, DataLoader *val_loader,
  ...
  // figure out the nu/ tmber of training steps we will run for
  int train_num_batches = max_steps; // passed in from command line
  if (train_num_batches == -1) {
    // sensible default is to train for exactly one epoch
    size_t ntok = train_loader->num_tokens;
